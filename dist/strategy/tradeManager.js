...
async function checkTrade(trade) {
    const currentPrice = await (0, market_1.getTicker)(trade.symbol);
    if (!currentPrice || !trade.id)
        return;
    const id = trade.id;
    if (!tpHitMap.has(id))
    ...
    if (trade.tp1Hit)
        hitTPs.add(1);
    if (trade.tp2Hit || trade.stopLoss === trade.entryPrice)
        hitTPs.add(2);
    if (trade.tp3Hit)
        hitTPs.add(3);
    ...
    if (tp3Hit && !hitTPs.has(3)) {
        hitTPs.add(3);
        (0, db_1.updateTradeTpHit)(id, 1);
        (0, db_1.updateTradeTpHit)(id, 2);
        (0, db_1.updateTradeTpHit)(id, 3);
    ...
    if (tp2Hit && !hitTPs.has(2)) {
        ...
        await (0, bot_1.broadcastTpHit)(trade, 2, currentPrice, true);
        logger_1.logger.info(`[TP2] ${trade.symbol} partial close + SL moved to TP1`);
        return;
    }
    ...
    if (tp1Hit && !hitTPs.has(1) && !trade.tp1ClosedSize) {
        ...
        await (0, bot_1.broadcastTpHit)(trade, 1, currentPrice, true);
        logger_1.logger.info(`[TP1] ${trade.symbol} partial close + SL moved to breakeven`);
    }
}
function updateTradeLifeMetrics(trade, currentPrice) {
    if (!trade.id)
        return;
    ...
    (0, db_1.updateTradeExcursion)(trade.id, parseFloat(maxProfitPercent.toFixed(4)), parseFloat(maxDrawdownPercent.toFixed(4)));
}
...
async function handlePartialClose(trade, tpLevel, currentPrice) {
    if (!trade.id)
        return;
    // Instrument sizing (fallback: ctVal=1, lotSz=1, minSz=1 -- never throws)
    const { ctVal, lotSz, minSz } = await (0, market_1.getInstrumentTradeParams)(trade.symbol);
    ...
    if (closeSize <= 0 || closeSize < minSz) {
        logger_1.logger.warn(`Partial close skipped for ${trade.symbol} TP${tpLevel}: closeSize=${closeSize} < minSz=${minSz}`);
    ...
    try {
        await (0, trading_1.closePartialPosition)(trade.symbol, trade.direction, actualCloseSize, currentPrice);
    }
    catch (err) {
        logger_1.logger.error(`Partial close failed for ${trade.symbol} TP${tpLevel}: ${err.message}`);
        (0, bot_1.sendErrorAlert)(err.message, `Partial close TP${tpLevel}: ${trade.symbol}`).catch(() => { });
    ...
    // Persist to DB
    (0, db_1.updateTradePartialClose)(trade.id, tpLevel, actualCloseSize, partialPnlUsdt, newRemainingSize);
    ...
    if (!config_1.config.trading.isLive) {
        (0, trading_1.updatePaperBalance)(partialPnlUsdt);
    }
    logger_1.logger.info(`TP${tpLevel} partial close: ${trade.symbol} ` +
...
async function moveStopToLevel(trade, newStopLoss) {
    if (!trade.id)
        return;
    (0, db_1.updateTradeStopLoss)(trade.id, newStopLoss);
    ...
    if (trade.slAlgoId) {
        try {
            await (0, trading_1.cancelAlgoOrder)(trade.symbol, trade.slAlgoId);
            logger_1.logger.info(`Cancelled old SL algo order ${trade.slAlgoId} for ${trade.symbol}`);
        }
        catch (err) {
            logger_1.logger.warn(`Failed to cancel old SL algo ${trade.slAlgoId} for ${trade.symbol}: ${err.message}`);
            (0, bot_1.sendErrorAlert)(err.message, `Cancel SL algo: ${trade.symbol}`).catch(() => { });
            // Don't abort -- proceed to place the new algo order regardless
        }
    }
    try {
        const result = await (0, trading_1.moveStopLossToBreakeven)(trade.symbol, trade.direction, orderSize, newStopLoss);
        if (!result.paper && trade.id) {
            (0, db_1.updateTradeSlAlgoId)(trade.id, result.orderId);
            trade.slAlgoId = result.orderId;
            logger_1.logger.info(`SL algo order saved: ${result.orderId} for ${trade.symbol}`);
        }
    }
    catch (err) {
        logger_1.logger.error(`Failed to move SL to ${newStopLoss} for ${trade.symbol}: ${err.message}`);
        (0, bot_1.sendErrorAlert)(err.message, `Move SL to ${newStopLoss}: ${trade.symbol}`).catch(() => { });
    }
}
// Convenience: move SL to entry price (breakeven).
async function moveStopToBreakeven(trade) {
    await moveStopToLevel(trade, trade.entryPrice);
}
async function handleClose(trade, exitPrice, status) {
    if (!trade.id)
        return;
    const isLong = trade.direction === 'LONG';
    ...
    if (trade.symbol.endsWith('-SWAP')) {
        let ctVal = 1;
        try {
            const info = await (0, market_1.getInstrumentInfo)(trade.symbol);
            ctVal = info?.ctVal ?? 1;
        }
        catch {
            logger_1.logger.warn(`Could not fetch ctVal for ${trade.symbol}, pnlUsdt computed with ctVal=1`);
        }
    ...
    const totalPnlUsdt = (trade.tp1PnlUsdt ?? 0) + (trade.tp2PnlUsdt ?? 0) + finalPnlUsdt;
    // -- Generate exit analysis --
    const { exitReason, exitAnalysis, improvements, errorTags } = await generateExitAnalysis(trade, status, exitPrice, pnlPercent);
    // -- Close in paper/live --
    ...
    try {
        await (0, trading_1.closePosition)(trade.symbol, trade.direction, closeSize, exitPrice);
    }
    catch (err) {
        logger_1.logger.error(`Failed to place close order for ${trade.symbol} #${trade.id}: ${err.message}`);
        (0, bot_1.sendErrorAlert)(err.message, `Close position: ${trade.symbol} #${trade.id}`).catch(() => { });
    ...
    (0, db_1.closeTrade)(trade.id, exitPrice, status, result, parseFloat(pnlPercent.toFixed(4)), parseFloat(totalPnlUsdt.toFixed(4)), exitReason, exitAnalysis, improvements, errorTags);
    // -- Update paper balance with FINAL portion only --
    // TP1 and TP2 partial PnLs were already applied in handlePartialClose.
    if (!config_1.config.trading.isLive) {
        (0, trading_1.updatePaperBalance)(finalPnlUsdt);
    }
    // -- Update risk counters --
    (0, riskManager_1.recordTradeResult)(pnlPercent);
    // -- Send Telegram notification --
    const updatedTrade = (0, db_1.getTradeById)(trade.id);
    if (updatedTrade) {
        await (0, bot_1.broadcastTradeClosed)(updatedTrade, improvements);
    }
    tpHitMap.delete(trade.id);
    logger_1.logger.info(`Trade ${trade.id} closed: ${status} | PnL: ${pnlPercent.toFixed(2)}% | ` +
        `Total USDT: ${totalPnlUsdt.toFixed(2)} (final: ${finalPnlUsdt.toFixed(2)})`);
}
async function generateExitAnalysis(trade, status, exitPrice, pnlPercent) {
    // Fetch current indicators for analysis
    let currentIndicators = null;
    try {
        const candles = await (0, market_2.getCandles)(trade.symbol, '1H', 50);
        currentIndicators = (0, indicators_1.computeIndicators)(candles, '1H');
    }
    catch { /* ignore */ }
    const exitReasons = [];
    const improvements = [];
        ...
        case 'closed_tp2':
            ...
            if (currentIndicators) {
                if (currentIndicators.rsi > 70)
                    exitReasons.push('RSI вошел в зону перегрева');
