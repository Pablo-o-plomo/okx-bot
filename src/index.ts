...
import { validateAndSetSymbols, getActiveSymbols } from './okx/symbolValidator';
...
async function bootstrap(): Promise<void> {
  ...
  // 2. Validate trading symbols against OKX API (removes invalid ones like TON-USDT-SWAP)
  await validateAndSetSymbols();
...
async function runSignalScan(): Promise<void> {
  ...
  if (symbols.length === 0) {
    logger.error('⛔ Нет активных символов — сканирование пропущено. Проверьте SYMBOLS в ENV.');
    return;
  }
